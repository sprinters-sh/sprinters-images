...
if [ "$SLIM" = "true" ]; then
  ...
  jq -M -C 'del(.android) | .toolcache = (.toolcache | map(select(.name != "CodeQL")))' "${PATH_ROOT}/../toolsets/toolset-2204${ARCH_SUFFIX}.json" > ${INSTALLER_SCRIPT_FOLDER}/toolset.json
else
  cp "${PATH_ROOT}/../toolsets/toolset-2204${ARCH_SUFFIX}.json" ${INSTALLER_SCRIPT_FOLDER}/toolset.json
fi
